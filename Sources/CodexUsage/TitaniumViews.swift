...
private struct GlassCardEdge: View {
  ...
  var body: some View {
    GeometryReader { proxy in
      ...
      ZStack {
        // The reference image has a faint full perimeter, then much stronger local
        // refractive highlights. Keep the base edge quiet so the hot segments read.
        RoundedRectangle(cornerRadius: style.radius, style: .continuous)
          .strokeBorder(.white.opacity(0.30), lineWidth: 0.70)

        RoundedRectangle(cornerRadius: max(1, style.radius - 1.2), style: .continuous)
          .strokeBorder(.white.opacity(style.innerRimOpacity), lineWidth: 0.65)
          .padding(1.2)
          .blendMode(.screen)

        // Tight optical halo hugging the edge rather than a uniform neon outline.
        RoundedRectangle(cornerRadius: style.radius, style: .continuous)
          .strokeBorder(edgeBaseColor.opacity(0.30), lineWidth: max(style.borderWidth, 0.9))
          .blur(radius: style.borderGlowBlur * 0.28)
          .blendMode(.plusLighter)

        if accent == .warm {
          edgeSegment(
            color: .white,
            size: CGSize(width: w * 0.36, height: 1.2),
            position: CGPoint(x: w * 0.22, y: 0.8),
            glowColor: Color(red: 1.0, green: 0.82, blue: 0.58),
            glowRadius: 7
          )

          edgeSegment(
            color: Color(red: 1.0, green: 0.78, blue: 0.47),
            size: CGSize(width: 1.25, height: h * 0.54),
            position: CGPoint(x: 0.8, y: h * 0.48),
            glowColor: Color(red: 1.0, green: 0.69, blue: 0.34),
            glowRadius: 11
          )

          edgeSegment(
            color: Color(red: 1.0, green: 0.79, blue: 0.50),
            size: CGSize(width: w * 0.33, height: 1.25),
            position: CGPoint(x: w * 0.20, y: h - 0.8),
            glowColor: Color(red: 1.0, green: 0.68, blue: 0.32),
            glowRadius: 12
          )

          edgeSegment(
            color: Color(red: 1.0, green: 0.91, blue: 0.76),
            size: CGSize(width: w * 0.22, height: 1.15),
            position: CGPoint(x: w * 0.88, y: h - 0.8),
            glowColor: Color(red: 1.0, green: 0.77, blue: 0.48),
            glowRadius: 8
          )
        } else {
          edgeSegment(
            color: Color(red: 0.94, green: 0.99, blue: 1.0),
            size: CGSize(width: w * 0.34, height: 1.2),
            position: CGPoint(x: w * 0.80, y: 0.8),
            glowColor: Color(red: 0.54, green: 0.80, blue: 1.0),
            glowRadius: 10
          )

          edgeSegment(
            color: Color(red: 0.64, green: 0.86, blue: 1.0),
            size: CGSize(width: 1.25, height: h * 0.66),
            position: CGPoint(x: w - 0.8, y: h * 0.51),
            glowColor: Color(red: 0.36, green: 0.72, blue: 1.0),
            glowRadius: 13
          )

          edgeSegment(
            color: Color(red: 0.74, green: 0.91, blue: 1.0),
            size: CGSize(width: w * 0.34, height: 1.25),
            position: CGPoint(x: w * 0.82, y: h - 0.8),
            glowColor: Color(red: 0.34, green: 0.72, blue: 1.0),
            glowRadius: 13
          )
        }
      }
  ...
  private func edgeSegment(
    ...
    Capsule(style: .continuous)
      .fill(color.opacity(style.borderSpecularOpacity))
      .frame(width: size.width, height: size.height)
      .position(position)
      .shadow(
        color: glowColor.opacity(style.borderGlowOpacity),
        radius: glowRadius
      )
      .shadow(
        color: .white.opacity(style.borderGlowOpacity * 0.34),
        radius: glowRadius * 0.42
      )
      .blendMode(.plusLighter)
  }
}

private struct R18GlassCardOptics: View {
  let style: VisualTokens.Card
  let accent: QuotaAccent
  ...
  var body: some View {
    GeometryReader { proxy in
      let size = proxy.size
      ZStack {
        // Quiet glass rim: the Golden V85 target uses a low-energy full perimeter and
        // localized optical energy instead of a uniform neon outline.
        RoundedRectangle(cornerRadius: style.radius, style: .continuous)
          .strokeBorder(.white.opacity(rimOpacity), lineWidth: 0.72)

        RoundedRectangle(cornerRadius: max(1, style.radius - 1.2), style: .continuous)
          .strokeBorder(.white.opacity(innerRimOpacity), lineWidth: 0.64)
          .padding(1.15)
          .blendMode(.screen)

        RoundedRectangle(cornerRadius: style.radius, style: .continuous)
          .strokeBorder(.white.opacity(coreOpacity), lineWidth: 0.82)
          .blendMode(.plusLighter)

        RoundedRectangle(cornerRadius: style.radius, style: .continuous)
          .strokeBorder(edgeColor.opacity(haloOpacity), lineWidth: 3.4)
          .blur(radius: 4.4)
          .blendMode(.plusLighter)

        if accent == .warm {
          warmOptics(size: size)
        } else {
          coolOptics(size: size)
        }
      }
      .frame(width: size.width, height: size.height)
    }
  ...
  private var rimOpacity: Double {
    accent == .warm
      ? VisualTokens.R19Optics.LeftCard.rimOpacity
      : VisualTokens.R19Optics.RightCard.rimOpacity
  }

  private var innerRimOpacity: Double {
    accent == .warm
      ? VisualTokens.R19Optics.LeftCard.innerRimOpacity
      : VisualTokens.R19Optics.RightCard.innerRimOpacity
  }

  private var coreOpacity: Double {
    accent == .warm
      ? VisualTokens.R19Optics.LeftCard.coreOpacity
      : VisualTokens.R19Optics.RightCard.coreOpacity
  }

  private var haloOpacity: Double {
    accent == .warm
      ? VisualTokens.R19Optics.LeftCard.haloOpacity
      : VisualTokens.R19Optics.RightCard.haloOpacity
  }

  private var edgeColor: Color {
    switch accent {
    case .warm: return Color(red: 1.0, green: 0.86, blue: 0.68)
    case .cool: return Color(red: 0.68, green: 0.88, blue: 1.0)
    }
  }

  @ViewBuilder
  private func warmOptics(size: CGSize) -> some View {
    // Broad lower-right warm bloom: translated from the user-approved V85 Golden proxy.
    Ellipse()
      .fill(
        RadialGradient(
          colors: [
            Color(red: 1.0, green: 0.86, blue: 0.64)
              .opacity(VisualTokens.R19Optics.LeftCard.focusOpacity),
            Color(red: 1.0, green: 0.61, blue: 0.22)
              .opacity(VisualTokens.R19Optics.LeftCard.focusOpacity * 0.44),
            .clear,
          ],
          center: .center,
          startRadius: 0,
          endRadius: VisualTokens.R19Optics.LeftCard.focusWidth * 0.48
        )
      )
      .frame(
        width: VisualTokens.R19Optics.LeftCard.focusWidth,
        height: VisualTokens.R19Optics.LeftCard.focusHeight
      )
      .rotationEffect(.degrees(Double(VisualTokens.R19Optics.LeftCard.focusRotation)))
      .blur(radius: VisualTokens.R19Optics.LeftCard.focusBlur)
      .position(
        x: VisualTokens.R19Optics.LeftCard.focusX,
        y: VisualTokens.R19Optics.LeftCard.focusY
      )
      .blendMode(.plusLighter)
    ...
    Capsule(style: .continuous)
      .fill(
        LinearGradient(
          colors: [
            Color(red: 1.0, green: 0.67, blue: 0.28).opacity(0.14),
            Color(red: 1.0, green: 0.79, blue: 0.48)
              .opacity(VisualTokens.R19Optics.LeftCard.beamOpacity),
            Color(red: 1.0, green: 0.94, blue: 0.82).opacity(0.26),
          ],
          startPoint: .bottom,
          endPoint: .top
        )
      ...
      .frame(
        width: VisualTokens.R19Optics.LeftCard.beamWidth,
        height: VisualTokens.R19Optics.LeftCard.beamHeight
      )
      .rotationEffect(.degrees(Double(VisualTokens.R19Optics.LeftCard.beamRotation)))
      .blur(radius: VisualTokens.R19Optics.LeftCard.beamBlur)
      .position(
        x: VisualTokens.R19Optics.LeftCard.beamX,
        y: VisualTokens.R19Optics.LeftCard.beamY
      )
      .blendMode(.plusLighter)

    Capsule(style: .continuous)
      .fill(.white.opacity(VisualTokens.R19Optics.LeftCard.detailBeamOpacity))
      .frame(
        width: VisualTokens.R19Optics.LeftCard.detailBeamWidth,
        height: VisualTokens.R19Optics.LeftCard.detailBeamHeight
      )
      .rotationEffect(.degrees(Double(VisualTokens.R19Optics.LeftCard.detailBeamRotation)))
      .blur(radius: VisualTokens.R19Optics.LeftCard.detailBeamBlur)
      .position(
        x: VisualTokens.R19Optics.LeftCard.detailBeamX,
        y: VisualTokens.R19Optics.LeftCard.detailBeamY
      )
      .blendMode(.screen)

    edgeSegment(
      color: .white,
      size: CGSize(width: size.width * 0.34, height: 1.0),
      position: CGPoint(x: size.width * 0.22, y: 0.7),
      glow: Color(red: 1.0, green: 0.80, blue: 0.54),
      radius: 5
    )
    edgeSegment(
      color: Color(red: 1.0, green: 0.79, blue: 0.49),
      size: CGSize(width: 1.05, height: size.height * 0.48),
      position: CGPoint(x: 0.7, y: size.height * 0.51),
      glow: Color(red: 1.0, green: 0.66, blue: 0.28),
      radius: 8
    )
    edgeSegment(
      color: Color(red: 1.0, green: 0.84, blue: 0.62),
      size: CGSize(width: size.width * 0.40, height: 1.1),
      position: CGPoint(x: size.width * 0.80, y: size.height - 0.7),
      glow: Color(red: 1.0, green: 0.67, blue: 0.30),
      radius: 9
    )
  }

  @ViewBuilder
  private func coolOptics(size: CGSize) -> some View {
    Ellipse()
      .fill(
        RadialGradient(
          colors: [
            Color(red: 0.78, green: 0.93, blue: 1.0)
              .opacity(VisualTokens.R19Optics.RightCard.focusOpacity),
            Color(red: 0.25, green: 0.65, blue: 1.0)
              .opacity(VisualTokens.R19Optics.RightCard.focusOpacity * 0.46),
            .clear,
          ],
          center: .center,
          startRadius: 0,
          endRadius: VisualTokens.R19Optics.RightCard.focusHeight * 0.46
        )
      )
      .frame(
        width: VisualTokens.R19Optics.RightCard.focusWidth,
        height: VisualTokens.R19Optics.RightCard.focusHeight
      )
      .blur(radius: VisualTokens.R19Optics.RightCard.focusBlur)
      .position(
        x: VisualTokens.R19Optics.RightCard.focusX,
        y: VisualTokens.R19Optics.RightCard.focusY
      )
      .blendMode(.plusLighter)

    Ellipse()
      .fill(Color(red: 0.26, green: 0.67, blue: 1.0).opacity(VisualTokens.R19Optics.RightCard.focus2Opacity))
      .frame(
        width: VisualTokens.R19Optics.RightCard.focus2Width,
        height: VisualTokens.R19Optics.RightCard.focus2Height
      )
      .blur(radius: VisualTokens.R19Optics.RightCard.focus2Blur)
      .position(
        x: VisualTokens.R19Optics.RightCard.focus2X,
        y: VisualTokens.R19Optics.RightCard.focus2Y
      )
      .blendMode(.plusLighter)

    Capsule(style: .continuous)
      .fill(Color(red: 0.82, green: 0.95, blue: 1.0).opacity(VisualTokens.R19Optics.RightCard.topBeamOpacity))
      .frame(
        width: VisualTokens.R19Optics.RightCard.topBeamWidth,
        height: VisualTokens.R19Optics.RightCard.topBeamHeight
      )
      .blur(radius: VisualTokens.R19Optics.RightCard.topBeamBlur)
      .position(
        x: VisualTokens.R19Optics.RightCard.topBeamX,
        y: VisualTokens.R19Optics.RightCard.topBeamY
      )
      .blendMode(.screen)

    Capsule(style: .continuous)
      .fill(Color(red: 0.38, green: 0.76, blue: 1.0).opacity(VisualTokens.R19Optics.RightCard.sideBeamOpacity))
      .frame(
        width: VisualTokens.R19Optics.RightCard.sideBeamWidth,
        height: VisualTokens.R19Optics.RightCard.sideBeamHeight
      )
      .blur(radius: VisualTokens.R19Optics.RightCard.sideBeamBlur)
      .position(
        x: VisualTokens.R19Optics.RightCard.sideBeamX,
        y: VisualTokens.R19Optics.RightCard.sideBeamY
      )
      .blendMode(.plusLighter)

    edgeSegment(
      color: Color(red: 0.94, green: 0.99, blue: 1.0),
      size: CGSize(width: size.width * 0.38, height: 1.0),
      position: CGPoint(x: size.width * 0.82, y: 0.7),
      glow: Color(red: 0.52, green: 0.80, blue: 1.0),
      radius: 7
    )
    edgeSegment(
      color: Color(red: 0.62, green: 0.86, blue: 1.0),
      size: CGSize(width: 1.1, height: size.height * 0.70),
      position: CGPoint(x: size.width - 0.7, y: size.height * 0.50),
      glow: Color(red: 0.30, green: 0.68, blue: 1.0),
      radius: 10
    )
    edgeSegment(
      color: Color(red: 0.72, green: 0.91, blue: 1.0),
      size: CGSize(width: size.width * 0.38, height: 1.1),
      position: CGPoint(x: size.width * 0.82, y: size.height - 0.7),
      glow: Color(red: 0.32, green: 0.70, blue: 1.0),
      radius: 10
    )
  }
  ...
  private func edgeSegment(
    ...
    Capsule(style: .continuous)
      ...
      .frame(width: size.width, height: size.height)
      .position(position)
      .shadow(color: glow.opacity(0.62), radius: radius)
      .blendMode(.plusLighter)
  }
}

private struct R18TitaniumRing: View {
  var body: some View {
    ZStack {
      Circle()
        .fill(.ultraThinMaterial)
...
private struct TitaniumRing: View {
  var body: some View {
    ZStack {
      Circle()
        .fill(.ultraThinMaterial)
        .opacity(VisualTokens.Header.ringGlassOpacity)
        ...
        .fill(
          RadialGradient(
            colors: [
              .white.opacity(0.08),
              .clear,
              Color.black.opacity(VisualTokens.Header.ringTintOpacity),
            ],
      ...
      // Soft refractive aura. It is deliberately weaker than the local hot arcs.
      Circle()
        .stroke(
          AngularGradient(
            colors: [
              .white.opacity(0.34),
              Color(red: 1.0, green: 0.79, blue: 0.49).opacity(0.42),
              Color(red: 0.67, green: 0.82, blue: 0.96).opacity(0.16),
              .white.opacity(0.28),
            ],
            center: .center,
            startAngle: .degrees(-118),
          ...
          lineWidth: VisualTokens.Header.ringStroke + 4.2
        )
        .blur(radius: VisualTokens.Header.ringGlow * 0.54)
        .opacity(VisualTokens.Header.ringRimGlowOpacity)
        .blendMode(.plusLighter)

      // Main glass bevel: metallic silver body, not a uniformly white neon ring.
      Circle()
        .stroke(
          AngularGradient(
            gradient: Gradient(stops: ringStops),
          ...
          lineWidth: VisualTokens.Header.ringStroke
        )
        .shadow(color: .black.opacity(0.30), radius: 1.2, y: 1)

      Circle()
        .stroke(.white.opacity(VisualTokens.Header.ringOuterHighlight * 0.74), lineWidth: 0.85)
        .padding(0.7)
        .blendMode(.screen)

      Circle()
        .stroke(Color.black.opacity(VisualTokens.Header.ringDarkEdge), lineWidth: 0.9)
        .padding(VisualTokens.Header.ringStroke + 0.45)

      Circle()
        .stroke(.white.opacity(VisualTokens.Header.ringInnerHighlight), lineWidth: 0.72)
        .padding(VisualTokens.Header.ringStroke + 1.45)
        .blendMode(.screen)

      // Warm right-side reflection found in the reference.
      Circle()
        ...
        .stroke(
          Color(red: 1.0, green: 0.79, blue: 0.49)
            .opacity(VisualTokens.Header.ringSpecularOpacity),
          style: StrokeStyle(
            lineWidth: VisualTokens.Header.ringSpecularWidth,
            lineCap: .round
          )
        )
        .shadow(
          color: Color(red: 1.0, green: 0.67, blue: 0.28)
            .opacity(VisualTokens.Header.ringWarmGlowOpacity),
          radius: VisualTokens.Header.ringGlow * 0.78,
          x: 2,
          y: 2
        )
        .blendMode(.plusLighter)

      // White/silver top-left reflection.
      Circle()
        .trim(from: 0.01, to: 0.25)
        .stroke(
          .white.opacity(VisualTokens.Header.ringSpecularOpacity),
          style: StrokeStyle(
            lineWidth: VisualTokens.Header.ringSpecularWidth * 0.88,
            lineCap: .round
          )
        )
        .shadow(color: .white.opacity(0.36), radius: VisualTokens.Header.ringGlow * 0.42)
        .blendMode(.screen)
    }
  }
  ...
  private var ringStops: [Gradient.Stop] {
    [
      .init(color: Color(red: 0.92, green: 0.94, blue: 0.95), location: 0.00),
      .init(color: Color(red: 0.98, green: 0.99, blue: 1.0), location: 0.18),
      .init(color: Color(red: 0.77, green: 0.80, blue: 0.82), location: 0.34),
      .init(color: Color(red: 0.86, green: 0.84, blue: 0.79), location: 0.47),
      .init(color: Color(red: 0.78, green: 0.66, blue: 0.50), location: 0.58),
      .init(color: Color(red: 0.52, green: 0.54, blue: 0.56), location: 0.70),
      .init(color: Color(red: 0.71, green: 0.78, blue: 0.83), location: 0.84),
      .init(color: Color(red: 0.90, green: 0.92, blue: 0.94), location: 1.00),
    ]
  }
}
